...
export function summarize(home: string): string {
  ...
  const last10 = events
    .slice(-10)
    .map(e =>
      `  ${e.ts} ${e.source} ${e.type}${e.action ? ":" + e.action : ""} ${e.repo ?? "-"} ${e.file ?? ""}`.trimEnd(),
    )
    .join("\n");
  return [
    headline,
    ...
    `by type:`,
    fmt(countBy(events, "type")),
    `by action:`,
    fmt(countBy(events, "action")),
    `by source:`,
    fmt(countBy(events, "source")),
    `by repo:`,
    fmt(countBy(events, "repo")),
    `last 10:`,
    last10 || "  (none)",
  ].join("\n");
}
