...
test("SessionStart (snake_case) emits session_start with cwd + model", async () => {
  ...
  const { code, stdout } = await runHookAt(
    "copilot",
    "on-session-start.sh",
    { hook_event_name: "SessionStart", session_id: sid, cwd: "/tmp/cq-proj", source: "startup", model: "gpt-5" },
    home,
  );
  expect(code).toBe(0);
  expect(stdout).toBe("");
...
test("SessionStart (camelCase) reads sessionId", async () => {
  ...
  const { code } = await runHookAt(
    "copilot",
    "on-session-start.sh",
    { hook_event_name: "SessionStart", sessionId: "s-camel", cwd: "/tmp/cq-proj", source: "new" },
    home,
  );
  expect(code).toBe(0);
...
test("UserPromptSubmit emits prompt and never stores the text", async () => {
  ...
  const { code, stdout } = await runHookAt(
    "copilot",
    "on-prompt.sh",
    { hook_event_name: "UserPromptSubmit", session_id: "p1", cwd: "/x", prompt: "my secret prompt" },
    home,
  );
  expect(code).toBe(0);
  expect(stdout).toBe("");
