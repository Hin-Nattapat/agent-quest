...
export const eventLocalDate = (ts: string): string => {
...
export const eventLocalHour = (ts: string): number => {
  return new Date(ts).getHours();
};

export const isNight = (ts: string): boolean => {
...
const isNextDay = (earlier: string, later: string): boolean => {
...
export const computeStreak = (dateKeys: string[], today?: string): IStreak => {
